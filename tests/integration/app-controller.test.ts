...
describe("애플리케이션 컨트롤러 통합", () => {
  it("슬롯을 저장한 뒤에만 상태와 선택을 변경한다", async () => {
  ...
  it("저장 실패 시 생성·편집·시간 생성 상태를 원래대로 유지한다", async () => {
    ...
    expect(
      await controller.updateSlot(initialState.slots[0]!.id, {
        name: "수정 슬롯",
        startDate: "2026-09-01",
        endDate: "2026-09-30",
      }),
    ).toMatchObject({ ok: false, error: { code: "save_failed" } });
    expect(await controller.generateNextTime(initialState.slots[0]!.id)).toMatchObject({
      ok: false,
  ...
  it("선택 슬롯 삭제 후 같은 위치의 다음 슬롯을 선택한다", async () => {
    ...
    expect(await controller.deleteSlot(first.id)).toEqual({ ok: true, value: undefined });
    expect(controller.getSnapshot()).toMatchObject({
      selectedSlotId: SECOND_SLOT_ID,
  ...
  it("생성 시간과 셸 형식을 조합해 같은 명령을 표시하고 복사한다", async () => {
    ...
    expect(await controller.generateNextTime(slotId)).toMatchObject({
      ok: true,
      value: { localDateTime: "2026-09-01T09:00:00", utcOffset: "+00:00" },
    });
    expect(await controller.setShellFormat("bash")).toEqual({ ok: true, value: undefined });
  ...
  it("백업을 내보내고 검증한 뒤 적용한다", async () => {
    ...
    expect(await controller.applyBackupImport(prepared.value)).toEqual({
      ok: true,
      value: undefined,
    });
    expect(controller.getSnapshot()).toMatchObject({
  ...
  it("잘못된 백업과 적용 저장 실패가 기존 상태를 보존한다", async () => {
    ...
    writer.shouldFail = true;
    expect(await controller.applyBackupImport(prepared.value)).toMatchObject({
      ok: false,
      error: { code: "save_failed" },
    });
